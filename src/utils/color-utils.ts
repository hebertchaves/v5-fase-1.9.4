...
export function analyzeComponentColors(node: QuasarNode): {
  ...
  if (node.attributes) {
    ...
    // Verificar classes para detectar cores
    if (node.attributes.class) {
      ...
      for (const className of classes) {
        if (className.startsWith('bg-')) {
          ...
          result.bgColor = colorName;
        } else if (className.startsWith('text-') && !className.match(/^text-([a-h][1-6]|body[12]|subtitle[12]|caption|overline)$/)) {
          const colorName = className.substring(5);
          result.textColor = colorName;
        }
      }
    }
  }
  
  // Inferir valores se necessário
